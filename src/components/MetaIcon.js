...
// Lightweight inline SVG set for list-row meta. 13×13, stroke-based, currentColor.
// Matches the sidebar icon vocabulary (Feather-style, 1.8 stroke).
const PATHS = {
  upload: (
    <>
  ...
  building: (
    <>
      ...
      <path d="M7 7h4M7 11h4M7 15h4M17 13h2M17 17h2" />
    </>
  ),
  mail: (
    <>
...
export default function MetaIcon({ name, className = 'meta-icon' }) {
  if (!PATHS[name]) return null;
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      {PATHS[name]}
    </svg>
  );
}
